...
public class MenuController {
    ...
    // Method for adding media to UI upon initialization:
    // Is run in App.java after setting the stage, as running from this object's
    // initialize method would lead to the stage being null
    ...
    private void addButtonToUI(AbstractMedia media, boolean isSeries, Image image) {
        ...
        } catch (Exception e) { // If no image is found, simply sets the name of the movie/series on the button
                                // instead
    ...
    private Image saveMedia(AbstractMedia media) {
        // TODO: update directory correspondingly upon media deletion
    ...
    // When all the text input fields are not null, enable the confirm button:
    @FXML
    public void handleInputChanged() {
        ...
        if (!(this.nameField.getText().equals("")) && !(this.ratingField.getText().equals("")) && selected &&
                !(this.runtimeSeasonsField.getText().equals("")) && !(this.releaseYearField.getText().equals(""))) {
    ...
    // Method needed by MenuControllerTest, as TestFX cannot operate FileChooser.
    public void setPrevImage(Image image) {
        this.prevImage = new Image(image.getUrl());
    }
